...
上述参数丢失已由部署文件和实机 API 定义确认。
公式服务器和策略 RPC 是不同读取路径，预检必须使用与生产 Gateway 一致的 `use_formula=False`。
本次本地针对性测试 38 项通过。代码 `347320e` 已推送并部署实盘适配器，用户重载模型后生效。
`002144.SZ` 在不补数的情况下由 241 条固定价零成交变成空返回，随后单日补数恢复真实 241 行。
另一个历史空样本 `001338.SZ` 只有 14:20–15:00 的 41 行，单日补数后恢复到 241 行。
原有 8 个成交额异常股票刷新同日历史后，差异也消失到既定每条 1 元容差内。
10 股共 2,410 条经生产 Gateway 同路由回读，与 MiniQMT 同日分区配对后 OHLC、成交量无差异，
成交额最大差不超过 1 元。此证据只覆盖目标样本；全市场正式数据仍未生成。
正式采集仍需补齐历史数据准备步骤，不能只对空返回触发下载。
